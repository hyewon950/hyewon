...
/**
 * Servlet implementation class ModifyCommunityCommit
 */
@WebServlet("/modifyCommunityCommit")
public class ModifyCommunityCommitServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ModifyCommunityCommitServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String encodePostKinds = URLEncoder.encode(multi.getParameter("postKinds"));
		
		if(session != null && (session.getAttribute("member")!= null)) {
			result = new CommunityService().modifyCommunity(postNo, subject, contentArea);
			if(result > 0) {
