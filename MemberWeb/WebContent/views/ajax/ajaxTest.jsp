...
<body>
	<h1>ajax 개요</h1>
	<p>
	Ajax는 Asynchronous Javascript and XML이란 용어로 서버로부터 데이터를 가져와 전체페이지를 새로 고치지 않고 일부만 로드할 수 있게 하는 기법
	비동기식 요청을 함.
	<br>
	</p>
	<h3>동기식 / 비동기식이란?</h3>
	...
	<h4>2.ajax(jquery)로 서버에 데이터 전송하기</h4>
	...
	<script>
		$("#jQbtn2").click(function(){
			...
			$.ajax({
				url:"/ajaxTest1",
				data:{msg :message},   /*첫번째 msg는 servlet의 msg임   */
				type:"get",
				success:function(){
	...
	<h4>4.서버로 전송겂 보내고 결과 문자열 받아서 처리</h4>
	...
	<br>
	'{'으로 시작하여 '}'로 끝나며 그 속에 데이터를 표기하고 
	'key:값(value)' 쌍으로 구성된다.</p>
	<pre>
		{
			"name":"이순신",
			"age":27,
	...
	<h4>6.서버로 전송값을 보내고 jsonArr로 결과 받아서 처리</h4>
	<p>유저 번호를 보내서 해당 유저를 가져오고, 없는 경우 전체리스트 가져오기</p>
	유저번호 입려기 :<input type="text" id="findNum"><br>
