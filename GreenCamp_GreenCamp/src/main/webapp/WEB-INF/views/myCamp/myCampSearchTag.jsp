...
<body id="top">
	...
	<section id="banner">
		...
		<form action="mycTagSearch.do" name="mycSearchForm" method="get">
			<fieldset>
				<legend style="color:white;">태그 선택</legend>
	...
	<div id="main">
		...
		<h1 id="myCampCnt" style="text-align: center">총
			${mycsearchpi.mycListCount } 곳의 숨겨진 명소가 등록되어있습니다</h1>
		<br>
		<c:if test="${mycsearchpi.mycListCount ==0}">
			<p style="text-align:center">제일 먼저 정보를 등록해보세요!</p>
		</c:if>
		<br>
...
<br><br>
		...
		<form action="mycTagSearch.do" name="mycSearchForm" method="get">
			...
			<!-- <input type="text" class="form-control" name="mycTag" id="mycTag"
				required="required" readonly> -->
				<input type="mycSearch" name="mycSearchValue" class="tagblock" id="mycTag" value="${mycSearch.mycSearchValue }" readonly>
				<!-- <input type="text" class="form-control" name="mycTag" id="mycTag"
								required="required" readonly> -->
		   <!--  <input type="mycTagSearch" class="form-control" name="mycSearchValue" name="mycTag" id="mycTag" required="required" readonly> -->
			<!--  <div id="result1"> </div> -->
		...
		<%-- 		 <c:forEach var="fileList" items="${fileList }">
			...
			<c:if test="${mycsearchpi.mycListCount ==0}">
					<p style="text-align:center;margin-top:-10px;">현재 등록된 게시물이 존재하지 않습니다.</p>
				</c:if>
			    ...
			    <c:forEach var="fileList" items="${fileList }">
			    	<c:choose>
			    		<c:when test="${fileList.mycNo eq myc.mycNo}">
			    			<a href="${mycdetail }" class="image fit" style="width:500xp;">
 ...
 --%>						
			...
			<td> 
				<div class="paging">
					...
					<!-- [이전] -->
				<c:if test="${mycsearchpi.mycCurrentPage <= 1 }">
					[이전] &nbsp;
				</c:if>
				<c:if test="${mycsearchpi. mycCurrentPage > 1 }">
					<c:url var="before" value="mycTagSearch.do">
					<c:param name="mycSearchValue" value="${mycSearch.mycSearchValue }"/>
					<c:param name="mycSearchCondition" value="${mycSearch.mycSearchCondition}"/>
					<c:param name="page" value="${mycsearchpi.mycCurrentPage - 1 }" />
				</c:url>
				<a href="${before }">[이전]</a> &nbsp;
				</c:if>
				
				<!-- [이전]과 [다음]사이 페이지 처리  -->
				<c:forEach var="p" begin="${mycsearchpi.mycStartPage }" end="${mycsearchpi.mycEndPage }">
					<c:if test="${p eq mycsearchpi.mycCurrentPage }">
						<font color="red" size="4"><b>[${p }]</b></font>
					</c:if>
					<c:if test="${p ne mycsearchpi.mycCurrentPage }">
						<c:url var="pagination" value="mycTagSearch.do">
							<c:param name="mycSearchValue" value="${mycSearch.mycSearchValue }"/>
				...
				<!-- [다음] -->
				<c:if test="${mycsearchpi.mycCurrentPage >= mycsearchpi.mycMaxPage }">
					[다음] &nbsp;
				</c:if>
				<c:if test="${mycsearchpi.mycCurrentPage < mycsearchpi.mycMaxPage }">
					<c:url var="after" value="mycTagSearch.do">
					<c:param name="mycSearchValue" value="${mycSearch.mycSearchValue }"/>
					<c:param name="page" value="${mycsearchpi.mycCurrentPage + 1 }" />
					</c:url>
					<a href="${after }">[다음]</a> &nbsp;
